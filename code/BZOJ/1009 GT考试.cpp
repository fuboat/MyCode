/*
	EXPERIENCES:
	1. For the Dynamic Programming on AC trie, we should follow the Fail pointer(失配指针) to update the ans;
*/
#include <cstdio>
#include <iostream>
#include <algorithm>
...
struct Trie{
	...
	void Solve(Matrix &a, LL d)
	// 递归写法 
	// 其实这就是用递归来进行矩阵快速幂
	// 但快速幂函数一般返回快速幂的结果,这里由于栈空间的限制,因此传引用
	// 不过功能大同小异
	{
		if(d == 1){
			a = start;
	...
	//二进制写法 
	{
		...
		int cnt = 0;
		while(d > 1){
			//I have to use Chinese......
			//由于矩乘递归会爆占空间(主要是耗在传值上面，你传的值是一个矩阵,当然,你可以传引用,这样就不会爆了),所以要用二进制扫描.
			//二进制位从高到低,对应位为1,当前矩阵为x,原矩阵为a,则进行x*x*a操作(x^2*a);
			//否则只进行x*x操作(x^2).
			c[++ cnt] = d & 1;
			d >>= 1;
		}
